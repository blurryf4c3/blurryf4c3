...
class tar_file extends archive {
    ...
    function create_tar() {
        ...
        foreach ($this->files as $current) {
            ...
            $block    = pack("a100a8a8a8a12a12a8a1a100a6a2a32a32a8a8a155a12", $current['name2'], sprintf("o", $current['stat'][2]), sprintf("o", $current['stat'][4]), sprintf("o", $current['stat'][5]), sprintf("1o", $current['type'] == 2 ? 0 : $current['stat'][7]), sprintf("1o", $current['stat'][9]), "        ", $current['type'], $current['type'] == 2 ? @readlink($current['name']) : "", "ustar ", " ", "Unknown", "Unknown", "", "", !empty($path) ? $path : "", "");
            $checksum = 0;
            for ($i = 0; $i < 512; $i++)
                $checksum += ord(substr($block, $i, 1));
            $checksum = pack("a8", sprintf("o", $checksum));
